...
function AddTestCaseTable({ setopenAddTestCaseModal, styles }) {
  ...
  const handleSubmit = (values, actions) => {
    ...
    axios
      .post(
        `/testcasetable?tablename=${values?.tablename}&description=${values?.description}&date=${values?.date}&precondition=${values?.precondition}&version=${values?.version}&assignedfolderId=${folderId}`,
        formData
      )
      .then((res) => {
  ...
  useEffect(() => {
    getTestEnvironments();
    let closeTableModal = (e) => {
      if (tableRef.current != null) {
        if (!tableRef.current.contains(e.target)) {
          return setopenAddTestCaseModal(false);
        }
      }
    };
    document.addEventListener("mousedown", closeTableModal);
    return () => {
      document.removeEventListener("mousedown", closeTableModal);
    };
  }, []);
  ...
  return (
    ...
    >
      <div className="bg-white p-8 w-6/12 rounded-md" ref={tableRef}>
